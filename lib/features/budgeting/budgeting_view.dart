            ...
            builder: (context, constraints) {
              if (constraints.maxWidth < 800) {
                return Column(
                  children: [
                    ImportOptionCard(
                      title: 'Import a default budget preset',
                      description: 'Choose from pre-made templates',
                      onTap: () => _showPresetsModal(context, viewModel),
                    ),
                    const SizedBox(height: AppTheme.spacingMd),
                    const ImportOptionCard(
                      title: 'Generate and import using AI',
                      description: 'Explain to Artificial Intelligence your desired budget to have its preset autogenerated',
                      isEnabled: false,
                    ),
                    const SizedBox(height: AppTheme.spacingMd),
                    ImportOptionCard(
                      title: 'Import a previously used budget',
              ...
              return Row(
                children: [
                  ...
                  const Expanded(
                    child: ImportOptionCard(
                      title: 'Generate and import with AI',
                      description: 'Explain to Artificial Intelligence your desired budget to have its preset autogenerated',
                      isEnabled: false,
                    ),
                  ),
            ...
            builder: (context, appContext, __) {
              return Column(
                ...
                children: [
                  ...
                  const SizedBox(height: AppTheme.spacingMd),
                  Text(
                    'Select a budget to adopt or manage your previous budgets',
                    style: AppTheme.bodyMedium
                        .copyWith(color: context.colors.textSecondary),
                  ),
                  const SizedBox(height: AppTheme.spacingLg),
                        ...
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: AppTheme.spacingXs),
                                Text(
                                  'Your saved budgets will appear here',
                                  style: AppTheme.bodyMedium.copyWith(
                                    color: context.colors.textTertiary,
                                  ),
                                ),
        ...
        builder: (dialogContext) {
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('You are currently working on a budget.'),
                const SizedBox(height: AppTheme.spacingSm),
                if (viewModel.saveValidationMessage != null) ...[
                  Container(
                    padding: const EdgeInsets.all(AppTheme.spacingXs),
                    ...
                    _handleSave(this.context, viewModel, then: () async {
                      ...
                      if (currentParticipant != null) {
                        ...
                        messenger.showSnackBar(
                          SnackBar(
                            content:
                                const Text('Budget adopted successfully!'),
                            backgroundColor: successColor,
                          ),
                        );
                      }
              ...
              TextButton(
                onPressed: () async {
                  ...
                  if (currentParticipant != null) {
                    ...
                    messenger.showSnackBar(
                      SnackBar(
                        content: const Text('Budget adopted successfully!'),
                        backgroundColor: successColor,
                      ),
                    );
                  }
  ...
  void _handleSave(
    ...
    if (!viewModel.canSave) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text(viewModel.saveValidationMessage ?? 'Cannot save Budget'),
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Enter a name for this Budget:'),
              const SizedBox(height: AppTheme.spacingSm),
              TextField(
  ...
  void _handleUpdate(BuildContext context, BudgetingViewModel viewModel) {
    if (!viewModel.canSave) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text(viewModel.saveValidationMessage ?? 'Cannot update Budget'),
          backgroundColor: context.colors.error,
        ),
      );
      return;
    ...
    if (currentTemplate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
              'No active Budget to update. Please save as a new Budget first.'),
